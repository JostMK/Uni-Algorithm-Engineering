...
static void fill_with_random_numbers(std::vector<uint32_t> &elements, const uint32_t max_value) {
...
int main(const int argc, char *argv[]) {
    ...
    for (uint32_t i = 0; i < element_count_b; i++) {
        ...
        std::vector<uint32_t> elements_b(count);
        fill_with_random_numbers(elements_b, element_count_a);

        sw.Restart();

        intersect_naive(elements_a, elements_b);
        const auto naive_time = std::round(sw.Split());
        ...
        const auto binary_time = std::round(sw.Split());
        ...
        const auto galopping_time = std::round(sw.Stop());
